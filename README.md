...
| `VK_TOKEN` | Токен сообщества ВКонтакте | `vk1.a.ABC123...` |
| `VK_GROUP_ID` | ID группы ВКонтакте (без минуса) | `123456789` |
...
```
http://<IP_BANANA_PI>:8000
```

#### Что вы увидите:

1. **Шапка страницы**:
   - Название проекта: "Autopilot Content"
   - Кнопки навигации: "Посты", "Генерация", "Статистика"

2. **Таблица постов**:
   | ID | Тема | Тип | Статус | Дата публикации | Действия |
   |----|------|-----|--------|-----------------|----------|
   | 1 | Как мыть 3D-формочки | польза | draft | - | [Одобрить] [Удалить] |
   | 2 | Тест формочек на прочность | вовлечение | approved | 2025-01-15 10:00 | [Опубликовать] |
   | 3 | Новогодняя скидка 20% | продажа | published | 2025-01-14 18:00 | [Просмотр] |

3. **Кнопки управления**:
   - **Генерировать пак**: Создаёт 7 постов на неделю
   - **Фильтры**: По статусу (draft/approved/published), по типу
   - **Пагинация**: Если постов больше 20

#### Примеры действий:

- **Одобрение поста**: Нажмите кнопку "Одобрить" → выберите дату и время публикации → пост получит статус `approved`.
- **Ручная публикация**: Для поста со статусом `approved` нажмите "Опубликовать сейчас" → пост будет отправлен в ВК немедленно.
- **Просмотр**: Кликните на тему поста → откроется модальное окно с полным текстом и изображением.
